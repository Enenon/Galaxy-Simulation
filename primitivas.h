...
void inicializarCorpos() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        // Atribui��o correta, elemento por elemento
        corpos[i][0] = massa;
...
// inicializarCorpos() � executado na main, porque a primitivas.h n�o pode realizar nenhum la�o ou algo do tipo
...
void desenhag() {
    ...
    for (int i = 0;i < n;i++) { // i � o que sofre a for�a
        vec3 p1(corpos[i][1], corpos[i][2], corpos[i][3]); vec3 v1(corpos[i][4], corpos[i][5], corpos[i][6]);
        vec3 a1(0, 0, 0);
        for (int j = 0; j < n; j++) {
            if (i != j && corpos[i][10] == 0 && corpos[j][10] == 0) {

                /// pr�xima tarefa: fazer as posi��es s� se alterarem depois de todas as itera��es acontecerem ///
                ...
                //a1 = aceleracao(F, p1, p2); // c�lculos mostraram que implementar direto na velocidade d� no mesmo q na acelera��o
                v1 = velocidade(F, v1, p1, p2);
                //v2 = velocidade(-F, m2, v2, p2, p1);
                // atualizar posi��es
                //p1.x = p1.x + v1.x*dt; p1.y = p1.y + v1.y*dt; p1.z = p1.z + v1.z*dt;

                //corpos[i][1] = p1.x; corpos[i][2] = p1.y; corpos[i][3] = p1.z; <----- agora � feito um loop s� para atualizar a posi��o dos corpos
...
void desenhaOld(float num) {
    ...
    glPushMatrix();
        ...
        ///desenhaCubo(10);
    glPopMatrix(); // essa fun��o retorna ao eixo de coordenadas anterior (que � o da identidade, nesse caso)
